  ...
  if ($payment_record == 0) {
    ...
    $record_transaction = $user->RecordTransation($staffid,$totalrevenue,$paidamount,$balance,$transactioncode,$ip_address,$date,$time);
      if ($record_transaction == 1) {
          header("Location: ../practitioner_performance_details?view=$staffid&s=Transaction Recorded!.");
          exit();
          }

  }
  else{
    header("Location: ../practitioner_record_payment?view=$staffid&amount=$totalrevenue&e=Similar Transaction Exists!.");
    exit();
  }
